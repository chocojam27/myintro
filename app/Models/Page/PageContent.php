...
class PageContent extends Model
{
    ...
    public static function addData($page_id,$request)
    {
        ...
        if ($request->field_type == 'video') {
            $data->content = 'uploads/others/no_video.jpg';
        }
    ...
    public static function updateData($request)
    {
        ...
        foreach ($pages as $page) {
            ...
            if (is_null($request['field_'.$page->id])) {
                if ($page->field_type == 'repeater') {
                    ...
                    foreach ($rq as $rk => $rv) {
                            ...
                            if (!empty($data->content)) {
                                if (array_key_exists(0,unserialize(base64_decode($data->content)))) {
                                    if (!empty(unserialize(base64_decode($data->content))[$rk][0]['field_value'])) {
                                        ...
                                        }elseif(strpos(unserialize(base64_decode($data->content))[$rk][0]['field_value'], 'uploads/cms/') !== false || strpos(unserialize(base64_decode($data->content))[$rk][0]['field_value'], 'uploads/others/no_video.jpg') !== false){
                                            // Delete file holds old file and first folder
                                            Helper::deleteFile(unserialize(base64_decode($data->content))[$rk][0]['field_value'],'cms');
                                        }
                                    }
                        ...
                        if ($field_value == 'uploads/others/no_video.jpg') {
                            if ($field_type != 'video') {
                                $field_value = '';
                            }
                        }
                        ...
                        for ($i=1; $i < $total; $i++) {
                            ...
                            if (array_key_exists($rk,$rq) && (is_string($rq[$rk]) || is_numeric($rq[$rk]) || is_null($rq[$rk]))) { // Not file or image
                                $field_value = $rq[$rk];
                            } else {
                                // Image/File section
                                $data = self::find($page->id);
                                ...
                                if (!empty($data->content)) {
                                    if (array_key_exists($rk,unserialize(base64_decode($data->content)))) {
                                        if (!empty(unserialize(base64_decode($data->content))[$rk][$i]['field_value'])) {
                                            if(is_array(unserialize(base64_decode($data->content))[$rk][$i]['field_value'])){
            ...
            if ($repeater == 'false') {
                if ($page->field_type == 'image' || $page->field_type == 'file' || $page->field_type == 'video') {
            ...
            if ($key != $request->key) {
                ...
                foreach ($value as $v) {
                    if ($v['field_type'] == 'file') {
                        foreach ($v['field_value'] as $key => $value) {
                            // Delete file holds old file and first folder
                            Helper::deleteFile($value,'cms');
                        }
                    }
                    if ($v['field_type'] == 'image' || $v['field_type'] == 'video') {
                        // Delete file holds old file and first folder
