																...
																<div class="repeater-children repeater-children-{!!$value->id!!} repeater-children-{!!$value->id.'-'.$k!!}" data-key="{!!$k!!}">
																	@foreach(unserialize($value->repeater_fields) as $krf => $rf)
																		<div class="form-group clearfix">
																			<label class="col-md-1 col-xs-12 control-label">{!! $rf['field_name'] !!}</label>
	...
	<script type="text/javascript">
		...
		if($(".file-simple").length > 0){
			...
			$('.file-simple').each(function(){
				// Check if file exist
				if($(this).data('value') != ''){
					var file = "<img src='{!!asset('uploads/cms')!!}/"+$(this).data('value')+"' class='file-preview-image' style='width:100%'>"
				}
				else{
					var file = "<img src='{!!asset('uploads/others/no_image_available.jpg')!!}' class='file-preview-image' style='width:100%'>"
				}
