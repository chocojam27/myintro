...
Form::macro('myDate', function ($name, $label="", $options=[], $default = null) {
    $label = ($label =='') ? '' : html_entity_decode(Form::label($name, $label));
    return "
        <div class='form-group'>
            ". $label .
              Form::date($name, $default, array_merge(["class" => "form-control"], $options)). "
        </div>
    ";
});
...
Form::macro('myRadioButton', function ($type, $name, $label="", $value='', $checked=false, $options=[]) {
    return "
        <div class='radio radio-".$type."'>
            ".
                Form::radio($name, $value, $checked, ['id' => $value] + $options)
            ."
            <label for='$value' class='peers peer-greed js-sb ai-c'>
                <span class='peer peer-greed'>$label</span>
            </label>
        </div>
    ";
